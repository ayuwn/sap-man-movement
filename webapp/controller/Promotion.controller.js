sap.ui.define([
    ...
    return BaseController.extend("bsim.hcmapp.man.movement.controller.Promotion", {
        ...
        onInit: function () {
            ...
            this._currentUser();
            this.getRouter().getRoute("promotion").attachPatternMatched(this._onPromotionRouteMatched, this);
            ...
            this.getView().byId("effectiveDateEndPromotion").setValue(sFormattedEndDate);
            this.getView().byId("effectiveDateStartPromotion").setValue(sFormattedDate);
            ...
            // this.getView().byId("tanggalBerakhirPromotion").setDateValue(oEndDate);
        ...
        _onPromotionRouteMatched: function (oEvent) {
            ...
            oModel.read("/RequestSet", {
                ...
                success: (oData) => {
                    ...
                    if (hasPendingRequest) {
                        MessageBox.error(
                            "A promotion request for this employee is already in progress. Please complete it before creating a new one.",
                    ...
                    this._getEmployeeData(EmployeeNumber)
                        .then(() => {
                            ...
                            if (this._sRequestId) {
                                ...
                                this.getRouter().navTo("promotion", {
        ...
        onEffectiveDateStartChange: function(oEvent) {
            ...
            var oStartDatePicker = oView.byId("effectiveDateStartPromotion");
            var oEndDatePicker = oView.byId("tanggalBerakhirPromotion");
            ...
            if (oStartDate instanceof Date && !isNaN(oStartDate)) {
                ...
                // Set value to tanggalBerakhirPromotion if valid
        ...
        _getEmployeeData: function (EmployeeNumber) {
            ...
            if (!oPromotionModel) {
                console.error("Promotion model is not available.");
                MessageBox.error("System error: Promotion model is not available.");
                return Promise.reject("Promotion model is not available.");
            }
            ...
            this.readEntityFromModel(oPromotionModel, sEmployeePath)
                .then((employeeResult) => {
                    if (!employeeResult) {
                    ...
                    return this.readEntityFromModel(oPromotionModel, sEmployeeDetailPath);
                })
        ...
        onSalaryAdjChange: function (oEvent) {
            ...
            if (sSelectedKey === "1") { // "Ya"
                ...
                this.byId("salaryAdjValuePromotion").setValue(""); // Clear the input value
            }
        },
        ...
        _postPromotionRequest: function () {
            ...
            const oPayload = {
                ...
                MassgDesc: this.byId("reasonTextPromotion").getText(),
                BeginDate: getFormattedDate("actDateStartPromotion"),
                EndDate: getFormattedDate("actDateEndPromotion"),
                ...
                PlansDest: this.byId("newPositionIdPromotion").getValue(),
                PlansDesc_Dest: this.byId("newPositionTextPromotion").getText(),
                PersgDest: this.byId("newEmployeeGroupIdPromotion").getValue(),
                PersgDestDesc: this.byId("newEmployeeGroupTextPromotion").getText(),
                PerskDest: this.byId("newEmployeeSubgroupIdPromotion").getValue(),
                PerskDestDesc: this.byId("newEmployeeSubgroupTextPromotion").getText(),
                WerksDest: this.byId("newPerAreaIdPromotion").getValue(),
                WerksDestDesc: this.byId("newPerAreaTextPromotion").getText(),
                BtrtlDest: this.byId("newPerSubAreaIdPromotion").getValue(),
                BtrtlDestDesc: this.byId("newPerSubAreaTextPromotion").getText(),
                Adjstmn: this.getView().getModel("dropdown").getProperty("/selectedSalaryAdj") || "",
                Zsalary: this.byId("salaryAdjValuePromotion").getValue() ? this.byId("salaryAdjValuePromotion").getValue().replace(/\D/g, '') : "0",
                Zdasar1: sSelectedAs,
                Zexholder: this.byId("employeeChangePromotion").getValue(),
                ZexholderDesc: this.byId("employeeChangeTextPromotion").getText(),
                Zdasar2: this.byId("basicConPromotion").getValue(),
                PlansAsl: this.byId("currentPositionIdPromotion").getValue(),
                PlansDescAsl: this.byId("currentPositionTextPromotion").getText(),
                WerksAsl: this.byId("currentPerAreaIdPromotion").getValue(), 
                WerksDescAsl: this.byId("currentPerAreaTextPromotion").getText(),
                BtrtlAsl: this.byId("currentPerSubAreaIdPromotion").getValue(),
                BtrtlDescAsl: this.byId("currentPerSubAreaTextPromotion").getText(),
                OuAsl: this.byId("currentUnitOrgIdPromotion").getValue(),
                OuDecAsl: this.byId("currentUnitOrgTextPromotion").getText(),
                OuDest: this.byId("newUnitOrgIdPromotion").getValue(),
                OuDescDest: this.byId("newUnitOrgTextPromotion").getText(),
                DivisiAsl: this.byId("currentDivisionIdPromotion").getValue(),
                DivisiDescAsl: this.byId("currentDivisionTextPromotion").getText(), 
                DivisiDest: this.byId("newDivisionIdPromotion").getValue(),
                DivisiDescDest: this.byId("newDivisionTextPromotion").getText(),
                PlansReqDesc: oPlansReqDesc,
                NamaKantorReq: oNamaKantorReq,
                DivisiDescReq: oDivisiDescReq
            };
        
            console.log("Promotion Payload:", oPayload);
            ...
            // Submit promotion data to the Promotion OData service
            const oModel = this.getOwnerComponent().getModel("Promotion");
            oModel.create("/RequestSet", oPayload, {
                success: (oData) => {
                    this._oBusyDialog.close();
                    MessageBox.success("Promotion request has been submitted successfully.", {
                        onClose: () => {
                ...
                error: (oError) => {
                    ...
                    if (oError) {
                        try {
                            ...
                            MessageBox.error("Error submitting promotion request: " + oError.message);
                        }
                    } else {
                        MessageBox.error("Unknown error occurred while submitting promotion request.");
                    }
                }
            });
        },
        ...
        handleCloseEmployeeChange: function(oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                this.byId("employeeChangePromotion").setValue(oSelectedItem.Key);
            }
        ...
        handleCloseReason: function(oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                this.byId("reasonPromotion").setValue(oSelectedItem.Key);
            }
            oEvent.getSource().getBinding("items").filter([]);
        },
        ...
        handleClosePosition: function(oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                this.byId("newPositionIdPromotion").setValue(oSelectedItem.Key);
            }
            oEvent.getSource().getBinding("items").filter([]);
        },
        ...
        handleCloseArea: function(oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                this.byId("newPerAreaIdPromotion").setValue(oSelectedItem.Key);
                ...
                this.byId("newPerSubAreaIdPromotion").setValue("");
            }
            oEvent.getSource().getBinding("items").filter([]);
        },
        ...
        _loadSubAreaData: function () {
            return new Promise((resolve, reject) => {
                const oModel = this.getOwnerComponent().getModel("Promotion"); // Use the global model
        ...
        handleCloseSubArea: function (oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                if (oSubAreaModel) {
                    ...
                    this.byId("newPerSubAreaIdPromotion").setValue(oSelectedItem.Key2);
                } else {
                    console.error("SubArea model is not initialized.");
                }
            }
        ...
        handleCloseGroup: function (oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                if (oGroupModel) {
                    ...
                    this.byId("newEmployeeGroupIdPromotion").setValue(oSelectedItem.Key);
                } else {
                ...
                if (oSubGroupModel) {
                    ...
                    if (oSubGroupData) {
                        ...
                        this.byId("newEmployeeSubgroupIdPromotion").setValue(""); 
        ...
        handleCloseSubGroup: function (oEvent) {
            ...
            if (aContexts && aContexts.length) {
                ...
                if (oSubGroupModel) {
                    ...
                    this.byId("newEmployeeSubgroupIdPromotion").setValue(oSelectedItem.Key2);
                } else {
                    console.error("SubGroup model is not initialized.");
                }
            }
